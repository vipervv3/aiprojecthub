...
-- Notification schedules table for AI intelligent notifications
CREATE TABLE notification_schedules (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  schedule_type VARCHAR NOT NULL CHECK (schedule_type IN ('morning', 'evening', 'custom', 'ai_triggered')),
  time_slot VARCHAR NOT NULL, -- e.g., "08:00", "18:00"
  days_of_week JSONB DEFAULT '[1,2,3,4,5]', -- 1=Monday, 7=Sunday
...
CREATE INDEX idx_ai_analysis_cache_hash ON ai_analysis_cache(input_hash);
...
-- AI analysis cache policies (system access only)
