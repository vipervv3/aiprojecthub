...
Don't include emojis. Be direct and helpful.`
  ...
  async processOneHourReminders(): Promise<void> {
    try {
      ...
      for (const task of tasks) {
        await this.sendTaskReminder(task, '1hour')
      }

      console.log(`✅ Processed ${tasks.length} 1-hour reminders`)
    } catch (error) {
  ...
  async processOneDayReminders(): Promise<void> {
    try {
      ...
      for (const task of tasks) {
        await this.sendTaskReminder(task, '1day')
      }

      console.log(`✅ Processed ${tasks.length} 1-day reminders`)
    } catch (error) {
  ...
  async processOverdueAlerts(): Promise<void> {
    try {
      ...
      for (const [userId, userTasks] of Object.entries(tasksByUser)) {
        // For now, send individual reminders
        // TODO: Consider sending a single summary email with all overdue tasks
        for (const task of userTasks.slice(0, 5)) { // Limit to 5 per user to avoid spam
          await this.sendTaskReminder(task, 'overdue')
        }
      }

      console.log(`✅ Processed overdue alerts for ${Object.keys(tasksByUser).length} users`)
    } catch (error) {
